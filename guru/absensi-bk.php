...
<!-- Tambahkan CSS Bootstrap Datepicker -->
<link rel="stylesheet"
...
<!-- Tambahkan jQuery dan Bootstrap Datepicker JS -->
                        ...
                        <table class="table table-striped table-bordered table-hover" data-page-length="50">
                            ...
                            <tbody>
                                <?php  
                                    ...
                                    $presensi_today_query = mysqli_query($mysqli, "SELECT * FROM presensi WHERE tahun='$sekolah[tahun]' AND semester='$sekolah[semester]' AND tanggal='$tanggal_presensi' AND id_kelas='$orderID'");
                                    
                                    // Buat array untuk menyimpan data presensi yang sudah ada
                                    $presensi_today = [];
                                    while($rpresensi = mysqli_fetch_array($presensi_today_query)){
                                        $presensi_today[$rpresensi['id_siswa']][$rpresensi['id_absen']] = $rpresensi['jumlah'];
                                    }
                                ...
                                <tr>
                                    ...
                                    <!-- Kolom input presensi berdasarkan absen yang lebih dari id_absen 1 -->
                                    <?php
                                    $absen = mysqli_query($mysqli, "SELECT * FROM absen WHERE id_absen > 1 ORDER BY id_absen ASC");
                                    while($rabsen = mysqli_fetch_array($absen)) {
                                        $jumlah = isset($presensi_today[$rkelas['id_siswa']][$rabsen['id_absen']]) ? $presensi_today[$rkelas['id_siswa']][$rabsen['id_absen']] : 0;
                                    ?>
                                    <td>
            ...
            foreach ($siswa as $id_siswa => $absensi) {
                foreach ($absensi as $id_absen => $jumlahInput) {
                    ...
                    if (!empty($jumlahInput) && intval($jumlahInput) > 0) {
                        ...
                        $cekabsen = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM presensi WHERE tahun='$sekolah[tahun]' AND semester='$sekolah[semester]' AND id_absen='$id_absen' AND id_siswa='$id_siswa' AND tanggal='$tanggal_presensi'"));
